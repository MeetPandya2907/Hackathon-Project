  ...
  try {
    ...
    try {
      ...
      if (error.code === 'auth/configuration-not-found') {
        console.error('Google sign-in failed - configuration not found. You need to enable Google sign-in in Firebase console.');
        return { 
          success: false, 
          message: "Google sign-in is not configured properly in Firebase. Please follow the setup guide to enable Google authentication."
        };
      }
...
function updateAuthUI() {
  ...
  if (googleBtn) {
    googleBtn.addEventListener('click', async () => {
      // Show some feedback while signing in
      googleBtn.disabled = true;
      googleBtn.innerHTML = '<span style="display:inline-block;width:20px;height:20px;border:2px solid #ccc;border-top-color:#666;border-radius:50%;animation:spin 1s linear infinite;margin-right:10px;"></span> Signing in...';
      
      // Define a helper function to reset the button
      const resetButton = () => {
        googleBtn.disabled = false;
        googleBtn.innerHTML = '<img src="https://upload.wikimedia.org/wikipedia/commons/5/53/Google_%22G%22_Logo.svg" alt="Google">Continue with Google';
      };
      ...
      if (result.success) {
        ...
        if (result.message.includes("not configured properly")) {
          // Create a more helpful error message
          const errorElement = document.createElement('div');
          errorElement.className = 'auth-error';
          errorElement.innerHTML = `
            <strong>Google Sign-In Not Configured</strong>
            <p>Your Firebase project needs Google authentication enabled.</p>
            <p><a href="GOOGLE_AUTH_SETUP.md" target="_blank">See setup instructions</a></p>
          `;
          
          // Find the right place to insert the error message
          const authInputs = document.querySelector('.auth-inputs') || googleBtn.parentNode;
          if (authInputs.previousElementSibling && authInputs.previousElementSibling.classList.contains('auth-error')) {
            authInputs.previousElementSibling.remove(); // Remove any existing error
          }
          authInputs.parentNode.insertBefore(errorElement, authInputs);
        } else {
